    ...
    IF wsstatus = ws_job.C_SUCCESS THEN
        ...
        FOR line_idx = 1 TO job.job_photo.getLength()
            CALL ws_job.uploadJobPhoto(
                    job.job_photo[line_idx].jp_code, job.job_photo[line_idx].jp_idx, job.job_photo[line_idx].jp_photo)
                RETURNING wsstatus
            IF wsstatus = ws_job.C_SUCCESS THEN
                DISPLAY SFMT("Photo Uploaded %1/%2 %3",
                    job.job_photo[line_idx].jp_code, job.job_photo[line_idx].jp_idx, job.job_photo[line_idx].jp_photo)
            ELSE
                DISPLAY SFMT("ws_job.ws_error.message=%1", ws_job.ws_error.message)
            END IF
...
FUNCTION do_pool_data()
    DEFINE wsstatus INTEGER
    DEFINE pool_data ws_customer.pool_dataResponseBodyType

    -- Exact pool does not matter as random data
    CALL ws_customer.pool_data("XXX") RETURNING wsstatus, pool_data.*
    IF wsstatus = ws_customer.C_SUCCESS THEN
        DISPLAY "Pool Data"
        DISPLAY util.Json.stringify(pool_data)
    ELSE
        DISPLAY SFMT("wsstatus=%1", wsstatus)
        DISPLAY SFMT("ws_customer.ws_error.message=%1", ws_customer.ws_error.message)
    END IF
